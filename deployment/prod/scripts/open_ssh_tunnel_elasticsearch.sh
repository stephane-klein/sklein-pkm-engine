...
if [ -z "${INSTANCE_PROD_ELASTICSEARCH_PORT}" ]; then
    echo "Error : INSTANCE_PROD_ELASTICSEARCH_PORT environment variable is not defined or empty"
    exit 1
fi

nohup ssh -L ${INSTANCE_PROD_ELASTICSEARCH_PORT}:127.0.0.1:${INSTANCE_PROD_ELASTICSEARCH_PORT} triton -N > /dev/null 2>&1 &
